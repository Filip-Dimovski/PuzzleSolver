...
class PuzzleSolution(Widget):
	...
	def solve_puzzle_algorithm(self):
		...
		for contours_part in contours:
			...
			_,_,w,h = rect
			if h>(400/number_contours)/10:
				contours_height.append(h)
			if w>(600/number_contours)/10:
		...
		for contours_part in contours:
			...
			res_feature_matches = self.feature_matches(part_name,full_puzzle_name,solution_name,contour_cnt)
	...
	def feature_matches(self,part_puzzle_name,full_puzzle_name,solution_name,part_number):
		try:
			...
			n_clusters = 5
			if n_clusters>len(list_points):
				n_clusters=len(list_points)

			kmeans = KMeans(n_clusters=n_clusters, init='k-means++', max_iter=300, n_init=10, random_state=0)
			pred_y = kmeans.fit_predict(list_points)

			leading_cluster = np.argmax(np.bincount(kmeans.labels_))

			leading_cluster_point = list( kmeans.cluster_centers_[leading_cluster, :] )

			cv2.putText(solution_puzzle, str(part_number), (int(leading_cluster_point[0]) , int(leading_cluster_point[1]) ) , cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0), lineType=cv2.LINE_AA) 
			logger.debug("PuzzleSolution:  future_matches match found at: {}".format(kmeans.cluster_centers_[leading_cluster, :]))
